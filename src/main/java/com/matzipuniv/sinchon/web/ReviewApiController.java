...
public class ReviewApiController {
    ...
    public ReviewResponseDto searchByNum(@PathVariable Long reviewNum){
        ...
        List<Long> imageNum = new ArrayList<>();
            //게시글 첨부파일 num 담을 List 객체 생성
        for(ImageResponseDto imageResponseDto : imageResponseDtoList)
            imageNum.add(imageResponseDto.getImageNum()); // 각 첨부파일 num을 imageNum에 추가

        return reviewService.searchByNum(reviewNum, imageNum); //게시글 num과 첨부파일 num 목록(imageNum) 전달받아 결과 반환
